...
class Producto:
    ...
    def __init__(self, codigo, descripcion, valor, cantidad, precio, precio_total):
        ...
        self.precio = precio            # Precio P/U
        self.precio_total  = precio_total# Precio total 

    # Este método permite modificar un producto.
    def modificar(self, nueva_descripcion, nuevo_valor, nueva_cantidad, nuevo_precio, nuevo_precio_total):
        self.descripcion = nueva_descripcion    # Modifica la descripción
        self.cantidad = nueva_cantidad          # Modifica la cantidad
        self.valor = nuevo_valor                # Modifica valor
        self.precio = nuevo_precio              # Modifica el precio
        self.precio_total = nuevo_precio_total  # Modifica el precio total
...
class Inventario:
    ...
    def agregar_producto(self, codigo, descripcion, valor, cantidad, precio, precio_total):
        producto_existente = self.consultar_producto(codigo)
        if producto_existente:
            print("Ya existe un producto con ese código.")
            return False
        nuevo_producto = Producto(codigo, descripcion, valor, cantidad, precio, precio_total)
        sql = f'INSERT INTO productos VALUES ({codigo}, "{descripcion}", "{valor}", {cantidad}, {precio}, {precio_total});'
    ...
    def consultar_producto(self, codigo):
        ...
        if row:
            codigo, descripcion, valor, cantidad, precio, precio_total = row
            return Producto(codigo, descripcion, valor, cantidad, precio, precio_total)
        return False

    def modificar_producto(self, codigo, nueva_descripcion, nuevo_valor, nueva_cantidad, nuevo_precio, nuevo_precio_total):
        producto = self.consultar_producto(codigo)
        if producto:
            producto.modificar(nueva_descripcion, nuevo_valor, nueva_cantidad, nuevo_precio, nuevo_precio_total)
            sql = f'UPDATE productos SET descripcion = "{nueva_descripcion}", valor = "{nuevo_valor}", cantidad = {nueva_cantidad}, precio = {nuevo_precio}, precio_total = {nuevo_precio_total} WHERE codigo = {codigo};' 
            print("-"*50)
    ...
    def listar_productos(self):
        ...
        print("Código\tDescripción\tValor\tCant\tPrecio\tPrecio Total")
        self.cursor.execute("SELECT * FROM productos")
        rows = self.cursor.fetchall()
        for row in rows:
            codigo, descripcion,valor, cantidad, precio, precio_total = row
            print(f'{codigo}\t{descripcion}\t{valor}\t{cantidad}\t{precio}\t{precio_total}')
        print("-"*50)
...
class Carrito:
    ...
    def agregar(self, codigo, cantidad, inventario):
        ...
        nuevo_item = Producto(codigo, producto.descripcion, producto.valor, cantidad, producto.precio, producto.precio_total)
    ...
    def mostrar(self):
        ...
        print("Código\tDescripción\tValor\tCant\tPrecio\tPrecio-Total")
        for item in self.items:
            print(f'{item.codigo}\t{item.descripcion}\t{item.valor}\t{item.cantidad}\t{item.precio}\t{item.precio_total}')
        print("-"*50)
...
mi_inventario.agregar_producto(1, 'Diodo Fast', '400V', 10, 4500,10*4500)
mi_inventario.agregar_producto(2, 'Capacitor No Pol', '10uF/250V', 5, 2500,2500*5)
mi_inventario.agregar_producto(3, 'Resistor', '33K', 15, 500,500*15)
mi_inventario.agregar_producto(4, 'Resistor', '10K', 100, 500,500*100)
